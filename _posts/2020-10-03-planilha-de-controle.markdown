...
<a href="{{ site.url }}/assets/anexos/Planilha_de_controle.xlsm"> Planilha de controle</a>

<a href="{{ site.url }}/assets/anexos/Manual_planilha.pdf">Manual de uso e manutenção</a>

A __senha para (des)proteger__ todas as abas é `123`. Minha recomendação é que tudo fique constantemente protegido, de forma a evitar que alguém apague as fórmulas por descuido. Caso queira mudar a senha, fique à vontade, mas não deixe de consultar antes o último tópico desse texto ("Fórmulas e VBA") e veja que isso pode ser mais difícil do que parece (mas, ainda sim, factível).
...
![Diagrama da unificação de arquivos](/assets/img/postagens/planilha/unificacao.png)
...
![Ficha de cadastro](/assets/img/postagens/planilha/Cadastro.PNG)
...
![Ficha de Passagens](/assets/img/postagens/planilha/Passagens.PNG)
...
![Ficha de Frequência](/assets/img/postagens/planilha/Frequencia.PNG)
...
![Ficha de abordagens](/assets/img/postagens/planilha/Abordagens.PNG)

### 5. Consulta Usuário

Ferramenta de consulta que procura __reunir todas as informações__ registradas sobre o usuário nas diferentes planilhas: informações cadastrais, frequência no setor e passagens concedidas. Esta planilha está configurada como relatório pronto para impressão, basta clicar em: `Arquivo > Salvar como > Tipo de arquivo > PDF`. Esse __relatório automático individual__ pode ser útil como anexo em discussões de caso e relatórios à justiça e outros setores. Para usá-lo, lembre-se de atualizar a logomarca do setor (se quiser), o endereço e meios de contato.

![Gráficos úteis](/assets/img/postagens/planilha/Consulta.png)
...
![Exemplo de relatório](/assets/img/postagens/planilha/Relatorio.png)
...
![Gráficos úteis](/assets/img/postagens/planilha/Relatoriob.PNG)
...
![Histórico do setor](/assets/img/postagens/planilha/Historico.PNG)
...
A maior parte das fórmulas é bem simples na verdade, com uso abusivo do `PROCV`. Exceção é o uso do `ÍNDICE` e das __funções matriciais__. Quando vejo a função abaixo (usada na aba de Consulta), por exemplo, eu juro que me dá dor de cabeça pensar por onde começar a entendê-la (e sim, fui eu que escrevi).
...
1. buscar com o `ÍNDICE` as pessoas atendidas no mês,
...
Pra finalizar, deixo abaixo os _scripts_ do _Visual Basic_ usados em cada planilha. A principal utilidade, caso queira ou precise, é saber onde trocar a senha nas planilhas de consulta e relatório mensal. Para isso, basta substituir todas as entradas `"123"` abaixo pela nova senha.

Para abrir o VBA no Excel, use o atalho `Alt+F11`, mas se você precisa dessa dica provavelmente irá pesquisar um pouco mais sobre ele.
